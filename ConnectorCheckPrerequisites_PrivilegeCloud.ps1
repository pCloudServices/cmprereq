...
Function Processors
{
	...
	try{
        ...
        If(($testComponents -like "*PSM - Privilege Session Manager*") -or ($testComponents -eq "All")){
            [int]$MachineProcessMin = 8
        }Else{
            [int]$MachineProcessMin = 4
        }
		
		$cpuNumber = (Get-CimInstance Win32_ComputerSystem).NumberOfLogicalProcessors
		if ($cpuNumber -ge $MachineProcessMin)
		{
			  $actual = $cpuNumber
			  $result = $True
		} 
		else 
		{
			  $actual = $cpuNumber
			  $result = $false
			  $errorMsg = "Less than minimum ($($MachineProcessMin)) cores detected"
		}
    ...
    if(![string]::IsNullOrEmpty($portalSubDomainURL)){
        Try{
            ...
            if($IdentityHeaderURL -like "*Error*"){
                ...
                if($IdaptiveResponse.Result.TenantId -eq $IdentityTenantId)
                {
                    $actual = $true
                    $result = $true
                ...
                {
                    # Tenant has Custom URL enabled, we don't support it yet.
					If($IdaptiveResponse.Result -like "*CannotSatisfyChallenge*")
                    {
                        $actual = $IdaptiveResponse.Result
                        $errorMsg = "Can't perform the check from this machine, check if you configured security zones or other blocks that prevent auth to Identity from this machine (try login from browser). If this is intended, you will have to perform this check manually (More info: https://docs.cyberark.com/identity/latest/en/content/getstarted/customdomain.htm#ManagetenantURLs)"
                    }
                    Elseif(-not($IdaptiveResponse.Result.TenantId -eq $IdentityTenantId))
					{
                        $actual = $false
                        $result = $false
                        $errorMsg = "Identity Tenant ID `"$($IdaptiveResponse.Result.TenantId)`" is different from current subdomain `"$($IdentityTenantId)`". It looks like you have configured customized URL in Identity Administration, please disable it and try again (more info: https://docs.cyberark.com/identity/latest/en/content/getstarted/customdomain.htm#ManagetenantURLs)."
                    }
                    Else
                    {
