...
$arrCheckPrerequisitesSIA = @(
"SIA-Assets", 
"SIA-BackendAccess",
"SIA-Portal",
"SIA-IOT",
"SIA-IOTCert"
"SIA-TunnelCertISRGX1"
)
...
#all
$arrCheckPrerequisites = @{General = $arrCheckPrerequisitesGeneral},@{CPM = $arrCheckPrerequisitesCPM},@{PSM = $arrCheckPrerequisitesPSM},@{SecureTunnel = $arrCheckPrerequisitesSecureTunnel},@{ConnectorManagement = $arrCheckPrerequisitesCM},@{SIA = $arrCheckPrerequisitesSIA}

$componentMapping = @{
    "SIA - Secure Infrastructure Access (DPA)" = $arrCheckPrerequisitesSIA
...
Function GPO-Local
{
	...
	try{
		...
		$global:gpopath = "C:\Windows\temp\GPOReport.xml"
		gpresult /f /x $gpopath *> $null
        ...
        Try{
            ...
            if($IdentityHeaderURL -like "*Error*"){
                ...
                Else
                {
                    ...
                    ElseIf($IdaptiveResponse.Result -like "*CannotSatisfyChallenge*")
                    {
                        $actual = $IdaptiveResponse.Result
                        $errorMsg = "Can't perform the check from this machine, check if you configured security zones or other blocks that prevent auth to Identity from this machine (try login from browser)."
                    }
                    Else
                    {
                        # catch all other errors, just display whatever we see in result.
                        $actual = $IdaptiveResponse.Result
                        $errorMsg = "Can't perform the check, try to login from a browser on this machine (URL: $($IdaptiveBasePlatformURL)) and see if the error is similar and fix it."
                    }                
                }
            }
...
Function SIA-Assets{
    ...
    $errorMsg = ""
    $actual = ""
    Write-LogMessage -Type Verbose -Msg "Starting SIA-Assets..."
    ...
    if(![string]::IsNullOrEmpty($portalSubDomainURL)){
        ...
        # Logic for SIA regions, if US east it doesn't require region code in the URL
        ...
        Write-LogMessage -Type Verbose -Msg "Finished SIA-Assets..."
        }
        Catch
        {
            $errorMsg = "Error: $(Collect-ExceptionMessage) $($respErr.message) $($_.exception.status) $($_.exception.Response.ResponseUri.AbsoluteUri)"
...
Function SIA-BackendAccess
{
    ...
    if(![string]::IsNullOrEmpty($region)){
	    ...
	    Write-LogMessage -Type Verbose -Msg "Runing SIA-BackendAccess"
...
Function SIA-Portal
{
    ...
    if(![string]::IsNullOrEmpty($region)){
	    ...
	    Write-LogMessage -Type Verbose -Msg "Runing SIA-Portal"
	    return Test-NetConnectivity -ComputerName $dpabeaddress -Port 443
    }
	Else
	{
...
Function SIA-IOT
{
    ...
    if(![string]::IsNullOrEmpty($region)){
	    ...
	    Write-LogMessage -Type Verbose -Msg "Runing SIA-IOT"
	    return Test-NetConnectivity -ComputerName $AwsIOTAddress -Port 443
    }
	Else
	{
...
Function SIA-TunnelCertISRGX1 {
    [OutputType([PsCustomObject])]
    param ()
    try {
        Write-LogMessage -Type Verbose -Msg "Starting SIA-TunnelCertISRGX1..."
        
        $actual    = $null
        $result    = $False
        $errorMsg  = ""
        $expected  = "CABD2A79A1076A31F21D253635CB039D4329A5E8"  # thumbprint (expires 2035)

        # Look for the certificate in the LocalMachine\Root store by thumbprint.
        $actualCert = Get-ChildItem -Path Cert:\LocalMachine\Root\* | Where-Object { $_.Thumbprint -eq $expected }
		
        if($actualCert -and ($actualCert.Thumbprint -eq $expected)) {
            $result   = $true
            $actual   = $expected
            Write-LogMessage -Type Verbose -Msg "Found the expected Let's Encrypt Root Certificate."
        }
        else
        {
            Write-LogMessage -Type Verbose -Msg "Couldn't find the `Let's Encrypt` Root Cert Authority. Checking if certificate auto-update is not blocked by GPO..."
            $regPath = "HKLM:\Software\Policies\Microsoft\SystemCertificates\AuthRoot"
            $keyName = "DisableRootAutoUpdate"

            if (Test-Path $regPath){
                $value = Get-ItemProperty -Path $regPath -Name $keyName -ErrorAction Stop | Select-Object -ExpandProperty $keyName
                # value = 0 Disabled (Good)
                # value = 1 Enabled (Bad)
                # Value = null - Not Configured (Good)
                
                if ($value -eq 1) {
                    $result = $False
                    $errorMsg = "GPO policy 'Turn off Automatic Root Certificates Update' is Enabled. ($($regPath+"\"+$keyName)), either disable this setting or install the Root Cert manually, link: https://docs.cyberark.com/ispss-deployment/latest/en/content/introduction/dpa_connector-machine-requirements.htm?tocpath=Prepare%20your%20environment%7CSystem%20and%20network%20requirements%7CSIA%20requirements%7C_____1#Softwareandotherrequirements"
                }
                elseif ($value -eq 0) {
                    # Registry exists but will not block auto update
                    $result = $true
                }
            }
            else
            {
                # Registry doesn't exist, meaning Not Configured, so not a blocker
                $result = $true
            }
        }
        Write-LogMessage -Type Verbose -Msg "Finished SIA-TunnelCertISRGX1"
    }
    catch {
        $errorMsg = "Error: $(Collect-ExceptionMessage $_.Exception)"
    }
...
else
{
	...
	try {
        ...
        If(($testComponents -like "*PSM - Privilege Session Manager*") -or ($testComponents -eq "All")){checkIfPSMisRequired}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCjkkZ/FjJ0BCa7
# T3N4YJjBONG+HpJmRUHPlFLupRe9SKCCGJkwggROMIIDNqADAgECAg0B7l8Wnf+X
...
# AQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIMjnkZAZOt8CPbOy2Cv3R/PQEVeNDDfw
# 6WQ7cd1bKBu/MA0GCSqGSIb3DQEBAQUABIICAGFDzFyGL4UzVUQbP8GISU3aat8s
# 9Fs3VxpFx6wisE3yMyIKaV2165Frxb3nzfVjpFmMxqLEEIvNODQCR2swlCDuTK3N
# gfR4bOr+Wls3PQN13xecU+Y9/ZqWTmZ31XhPgvNmGplYD6voVQkVQXCB/e1QE9Kj
# TXebF1mMTi6N3d+v1IPXjOJ/p3wdWpV7w8NQSppyfwUskE1mHR0tdYds0N8NMyLt
